...
raw <- read.csv(here("IRMS Data/Raw IRMS Data Files", "COMPASS_Synoptic_NExpt_GWI_raw2.csv"))
head(raw)
...
write.csv(samples, file = here("Processed Data","COMPASS_NExpt_N2Calc_FullOutput_GWI2.csv"))  #this will give you everything 

#write.csv(output, file="output.csv")  #this is if you want the samples, areas, del value, and corrected 29 & 30 N2
write.csv(finaloutput, file = here("Processed Data","COMPASS_NExpt_N2Calc_Output_GWI2.csv"))  #this will give you just the sample names and the corrected 29 & 30
###########################################################
