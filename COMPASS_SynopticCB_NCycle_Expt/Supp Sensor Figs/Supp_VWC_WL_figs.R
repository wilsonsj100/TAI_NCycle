...
# combine both year data frames into one 

all <- rbind(alldat, alldat1)
all$year <- lubridate::year(all$TIMESTAMP)
head(all)


# plot all the data together 
boxes <- ggplot(data=all, aes(x=Zone, y=Value, fill=Zone))+
  geom_boxplot() + 
  facet_wrap(Site ~ year) + 
  theme_classic() + 
  labs(title= "", x=" ", y= expression(paste("Soil VWC (m"^3*"/ m"^3*")"))) +
  scale_fill_manual(values=compass_coolors) +
  theme(panel.background = element_rect(colour = "black", linewidth  =1.2), 
        legend.position = "NONE", 
        legend.title= element_blank(), 
...
########## This is the soil conductivity across sites in wetlands for Sierra ################
head(VWC23raw)

#subset by the soil moisture at 10cm 
EC23 <-  as.data.frame(subset(VWC23raw, research_name == c("soil_EC_10cm", "soil_EC_15cm"), select = Site:F_OOS))

#remove any data rows that are out of bounds 
EC23 <- as.data.frame(subset(EC23, !F_OOB == "1"))
head(EC23)

alldat2 <- EC23
head(alldat2)

alldat2$Site <- as.factor(alldat2$Site)
levels(alldat2$Site)


alldat2$Zone <- as.factor(alldat2$Plot)
levels(alldat2$Zone)
levels(alldat2$Zone) <- list(Upland = "UP", Transition = "TR", Wetland = "W", Upland = "SWAMP")
levels(alldat2$Zone)

alldat2 <- as.data.frame(subset(alldat2, Zone == "Wetland"))

alldat2 <-  alldat2 %>%  
  mutate(Site = fct_relevel(Site, "SWH", "GCW", "MSM", "GWI"))

salinitycoolors <- c("lightskyblue1", "steelblue2", "steelblue3" ,"royalblue4")

#plot as a bar plot across the sites 
box2 <- ggplot()+
  geom_boxplot(data=alldat2, aes(x=Site, y=Value, fill=Site, col= Site, alpha=0.7)) + 
  #facet_wrap(~Site, ncol = 2) + 
  #facet_grid(~factor(Site, levels=c("SWH", "GCW","MSM", "GWI"))) +
  theme_classic() + 
  labs(title= " ", x=" ", y= expression(paste("Soil EC (mS/cm)"))) +
  #scale_fill_manual(values=compass_coolors) +
  #scale_x_discrete(labels=c("Upland", "Transition", "Wetland")) +
  #scale_color_manual(values=c( "grey","grey", "grey","grey")) + 
  scale_color_manual(values=salinitycoolors) + 
  #scale_fill_manual(values=c( "grey","grey", "grey","grey")) +
  scale_fill_manual(values=salinitycoolors) +
  coord_flip() + 
  theme(panel.background = element_rect(colour = "black", linewidth  =1.2), 
        legend.position = "NONE", 
        legend.title= element_blank(), 
