...
setwd("S:/Biogeochemistry/People/Wilson (Steph)/Compass N Cycle Experiment/Full Incub. Experiment/Data/MIMS Data/Processed Data")
...
setwd("S:/Biogeochemistry/People/Wilson (Steph)/Compass N Cycle Experiment/Full Incub. Experiment/Data/MIMS Data/Rates & Figures")
...
Gavg <- G1 %>% 
  ...
  summarise(mean = mean(NH4_nmole, na.rm=TRUE))
head(Gavg)
...
#GCrew
Gres <- aov(NH4_Rate_g_hr ~ Zone, data = GRates)
summary(Gres)                                         # Zone: p=0.08
#TukeyHSD(Gres)                                        # UP-TR ; WC-TR ; WC-UP 
#tukey.plot.test<-TukeyHSD(Gres)
#plot(tukey.plot.test, las = 1)

#MSM
Mres <- aov(NH4_Rate_g_hr ~ Zone, data = MRates)
summary(Mres)                                         # Zone: p=0.549
#TukeyHSD(Mres)                                        # UP-TR ; WC-TR ; WC-UP 
#tukey.plot.test<-TukeyHSD(Gres)
#plot(tukey.plot.test, las = 1)

#GWI
GWres <- aov(NH4_Rate_g_hr ~ Zone, data = GWRates)
summary(GWres)                                         # Zone: p=0.0003
TukeyHSD(GWres)                                        # UP-TR Sig; WC-TR sig; WC-UP sig
tukey.plot.test<-TukeyHSD(GWres)
plot(tukey.plot.test, las = 1)

#SWH
Sres <- aov(NH4_Rate_g_hr ~ Zone, data = SRates)
summary(Sres)                                         # Zone: p=0.0008
TukeyHSD(Sres)                                        # UP-TR Sig; WC-TR Sig; WC-UP not
...
RatesAvg <- Rates %>% 
  group_by(Site, Zone) %>% 
  summarise(mean = as.numeric(mean(NH4_Rate_g_hr, na.rm=TRUE)),
...
Gavg <- ggplot(data=G2, aes(x=Zone, y=mean, fill=Zone)) +
  ...
  ylim(-1,25)+
  theme_classic() +
  scale_fill_manual(values=c(compass_coolors2a)) + 
  scale_x_discrete(labels=c("Upland Forest", "Transition", "Wetland")) +
...
####################### NOTES & COLORS ##############################
# color options 
# c('lightsalmon4','lightsalmon3', 'darkseagreen4')
# c('brown4','peachpuff3', 'darkslategray')
#   scale_fill_manual(values=c('#3B1802','#D45113', '#264027')) +
#  scale_fill_manual(values=c('#0F1A10','#8D8D92', '#152266')) + 

##################################################################

### removed because these data can be negative and therefore are not logged 

########## check with log transformed data  - negatives produce NANs ##########LOOK AT THIS FURTHER 
Rates$LogN2 <- log(Rates$NH4_Rate_g_hr)
qqnorm(Rates$LogN2) #technically the data cannot be zero so they are log normal, but look pretty normal here.... 
qqline(Rates$LogN2)

#GCReW
GRates$LogN2 <- log(GRates$NH4_Rate_g_hr)
#do a variance test to check homogeneity of variance 
...
#MSM
MRates$LogN2 <- log(MRates$NH4_Rate_g_hr)
#do a variance test to check homogeneity of variance 
...
#GWI 
GWRates$LogN2 <- log(GWRates$NH4_Rate_g_hr)
#do a variance test to check homogeneity of variance 
...
#SWH
SRates$LogN2 <- log(SRates$NH4_Rate_g_hr)
#do a variance test to check homogeneity of variance 
